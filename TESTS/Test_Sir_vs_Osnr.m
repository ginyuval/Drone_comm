%% Test 2: Input SIR vs. Output SNR
% clear; clc; close all;
addpath(genpath('../')); % Ensure access to parent folders

%% 1. Parameters Setup
jammerTypeToTest = 'multitone'; 

% Define range of SIR values to test [dB]
% From strong interference (-30 dB) to weak interference (10 dB)
sir_values_dB = -30 : 5 : 10; 
results_oSNR = zeros(size(sir_values_dB));

fprintf('Starting SIR Test with %s Jammer...\n', jammerTypeToTest);

%% 2. Loop
for i = 1:length(sir_values_dB)
    % Initialize params exactly as in main.m (or the provided base function)
    gl_params = get_main_params(jammerTypeToTest);
    
    % Override the specific parameter for this test
    gl_params.SIR_in_dB = sir_values_dB(i);
    
    % Run Simulation
    avgSNR = run_simulation_core(gl_params);
    results_oSNR(i) = avgSNR;
    
    fprintf('Run %d/%d: SIR=%d dB -> oSNR=%.2f dB\n', ...
        i, length(sir_values_dB), sir_values_dB(i), avgSNR);
end

%% 3. Plot
figure(1);
hold on;
plot(sir_values_dB, results_oSNR, '-s', 'LineWidth', 2, 'MarkerFaceColor', 'b');
xlabel('Input SIR [dB]');
ylabel('Average Output SNR [dB]');
title(['Input SIR vs. Output SNR (' jammerTypeToTest ')']);
grid on;
...
function gl_params = get_main_params(jType)
    ...
    gl_params.SIR_in_dB = -20; % Default, overridden in loop
    
    % DOAs
    gl_params.theta_desired_deg = -35;
    ...
    gl_params.guardIntervalSec = 10e-6;
